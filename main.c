...
PONT buscar(PONT raiz, int valor) {
    // COMPLETAR
    if (raiz == NULL) return NULL;// Retorna ponteiro para nó com chave == valor ou NULL se não existir.
    // Usar a lógica de BST:
    if (valor < raiz->chave) return buscar(raiz->esq, valor);//  - se valor < raiz->chave => buscar à esquerda
    if (valor > raiz->chave) return buscar(raiz->dir, valor); //  - se valor > raiz->chave => buscar à direita
    return raiz;//  - se valor == raiz->chave => retorna raiz
    // provisório
}
...
PONT inserir(PONT raiz, int valor) {
    // COMPLETAR
    if (raiz == NULL) return criarNo(valor);// Se raiz == NULL => cria nó
    if (valor < raiz->chave) {
        raiz->esq = inserir(raiz->esq, valor);}// Se valor < raiz->chave => raiz->esq = inserir(raiz->esq, valor)
    else if (valor > raiz->chave) {
        raiz->dir = inserir(raiz->dir, valor);}// Se valor > raiz->chave => raiz->dir = inserir(raiz->dir, valor)
    else {
        raiz->contador++;}// Se valor == raiz->chave => incrementa raiz->contador
...
void exibirInOrder(PONT raiz) {
    ...
    if (raiz != NULL) {
        ...
        exibirInOrder(raiz->dir);// Percurso InOrder: esq -> (raiz->chave impresso contador vezes) -> dir
    }
}
...
int contarNos(PONT raiz) {
    // COMPLETAR
    if (raiz == NULL) return 0;// Se raiz==NULL => 0
...
int contarTotalElementos(PONT raiz) {
    if (raiz == NULL) return 0;// COMPLETAR
    return raiz->contador + contarTotalElementos(raiz->esq) + contarTotalElementos(raiz->dir);// soma = raiz->contador + subárvores
    // provisório
}
...
int kEsimoMenor(PONT raiz, int k) {
    ...
    int esq = contarTotalElementos(raiz->esq); // Contagem correta da subárvore esquerda
    
    if (k <= esq) return kEsimoMenor(raiz->esq, k);
    if (k > esq + raiz->contador) return kEsimoMenor(raiz->dir, k - (esq + raiz->contador));
